...
func TestTableRow_GetPartKeyValues(t *testing.T) {
	row := tableReader.GetRow("messagesTag")
...
func TestTable_ReadSortKeyValues(t *testing.T) {
	...
	sortKeyValue, err := tableReader.ReadFirstSortKeyID(ctx, row, partKeyValue)
	if err != nil {
		t.Errorf("ReadFirstSortKeyID failed: %v", err)
	...
	sortKeyValue, err = tableReader.ReadLastSortKeyID(ctx, row, partKeyValue)
	if err != nil {
		t.Errorf("ReadLastSortKeyID failed: %v", err)
...
func TestTable_ReadTextValue(t *testing.T) {
	row := tableReader.GetRow("messagesTag")
	if !row.IsValid() {
		t.Fatalf("messagesTag row not found")
	}
...
func NewThingTable(dbClient *dynamodb.Client, env string) Table[VersionableThing] {
	...
	return Table[VersionableThing]{
		...
		TableName:  "VersionableThing-" + env,
		TTL:        true,
